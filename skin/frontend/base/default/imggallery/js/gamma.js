...
$j.extend( $j.expr[':'], {
	...
	inViewport : function( el ) {
		...
		var scrollTop = ( document.documentElement.scrollTop || document.body.scrollTop ),
			elOffsetTop = $j( el ).offset().top,
			elH = $j( el ).height()
			winH = ( window.innerHeight && window.innerHeight < $j( window ).height() ) ? window.innerHeight : $j( window ).height();
...
var Gamma = (function() {
	...
	var $window = $( window ),
		...
		init = function( settings, callback ) {
			...
			Gamma.settings = $j.extend( true, {}, defaults, settings );
		...
		_config = function() {
			...
			Gamma.container = $j( '#gamma-container' );
            //console.log(Gamma.container);
		...
		_createSingleView = function() {
			...
			if( !Gamma.singleview ) {
				...
				$j( '<div class="gamma-single-view"><div class="gamma-options gamma-options-single"><div class="gamma-buttons"><button class="gamma-btn-close"></button></div></div></div>' )
		...
		_createSingleViewNavigation = function() {
			...
			if( Gamma.itemsCount > 1 ) {
				...
				Gamma.svplay = $j( '<button class="gamma-btn-ssplay"></button>' ).insertAfter( Gamma.svclose );
				Gamma.nav = $j( '<nav class="gamma-nav"><span class="gamma-prev"></span><span class="gamma-next"></span></nav>' ).appendTo( Gamma.singleview );
		...
		_layout = function( $items ) {
			...
			$items.each( function() {
				...
				$j( '<div/>' ).addClass( 'gamma-description' ).html( description ).insertAfter( $picEl );
		...
		_singleviewitem = function( $item, anim ) {
			...
			if( anim ) {
				...
				Gamma.fly = $j( '<img/>' ).attr( 'src', $img.attr( 'src' ) ).addClass( 'gamma-img-fly' ).css( {
					width : $img.width(),
					height : $img.height(),
			...
			var	finalConfig = _getFinalImgConfig( {
					...
					wrapper : { width : jQuery($window).width() - Gamma.svMargins.horizontal, height : jQuery($window).height() - Gamma.svMargins.vertical },
			...
			setTimeout( function() {
				...
				if( !anim ) {
					...
					var styleCSS = {
							width : finalSizePosition.width,
							height : finalSizePosition.height,
							left : finalSizePosition.left + jQuery($window).scrollLeft() + Gamma.svMargins.horizontal / 2,
							top : finalSizePosition.top + jQuery($window).scrollTop() + Gamma.svMargins.vertical / 2
						}, 
		...
		_loadSVItemFromGrid = function( data, position, src ) {
			...
			Gamma.svImage = $j( '<img/>' ).load( function() {
		...
		_closesingleview = function() {
			...
			if( !$item.is( ':inViewport' ) ) {
				...
				var diff = $document.height() - jQuery($window).height();
                ...
                jQuery($window).scrollTop( wst );

			}

			var l = Gamma.svImage.position().left + jQuery($window).scrollLeft(),
			...
			switch( type ) {
				...
				case 'window' : 
					...
					if( Gamma.settings.historyapi ) {
						...
						jQuery($window).on( 'statechange.gamma', function() {
                    ...
                    jQuery($window).on( 'smartresize.gamma', _resize );
		...
		_applyAnimation = function( el, styleCSS, speed, cond, fncomplete ) {
			...
			$j.fn.applyStyle = cond ? $j.fn.css : $j.fn.animate;
			...
			el.stop().applyStyle( styleCSS, $j.extend( true, [], { duration : speed + 'ms', complete : fncomplete } ) );
